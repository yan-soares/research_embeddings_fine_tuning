...
NLI_CSV_PATH = os.path.join(PROJECT_ROOT, "data/nli_optimized_for_layer_search_50k.csv")
...
class DynamicFusionLayer(nn.Module):
    def __init__(self, num_layers, mode='weighted_mean'):
        super().__init__()
        self.mode = mode
    ...
    def forward(self, all_hidden_states):
        ...
        if self.mode == 'weighted_sum':
            # SOMA PONDERADA (Seu objetivo atual)
            # Permite pesos livres (negativos, gigantes, etc)
            weights = self.layer_weights
        else:
            # MÉDIA PONDERADA (Softmax)
            # Força soma = 1 (distribuição de probabilidade)
...
class TaskSpecificModel(nn.Module):
    def __init__(self, model_name, num_classes, pooling_type, type_fusion, mode_fusion): # Novo parâmetro
        ...
        if type_fusion == 'dynamic_layer':
            self.fusion = DynamicFusionLayer(num_layers=self.n_layers, mode=mode_fusion)
            print('Fusão: Dynamic Layer')
        elif type_fusion == 'cnn_1d':
            self.fusion = Conv1dFusionLayer(num_layers=self.config.num_hidden_layers)
            print('Fusão: CNN 1D')
        else:
            self.fusion = DynamicFusionLayer(num_layers=self.n_layers, mode=mode_fusion)
            print('Fusão: Dynamic Layer')
...
def train_task_engine(task_name, seed_idx=0, total_seeds=5): # Adicionei argumentos para o log ficar bonito
    ...
    model = TaskSpecificModel(CONFIG['model_name'], num_classes=num_classes, pooling_type=CONFIG['pooling_type'], type_fusion=CONFIG['type_fusion'], mode_fusion=CONFIG['mode_fusion']).to(CONFIG['device'])
    ...
    for epoch in epochs_bar:
        ...
        if metric > best_val_metric:
            ...
            if patience_counter >= PATIENCE:
                ...
                log_this(f"Task: {task_name} , Seed: {seed_idx+1} -> Early Stopping na época {epoch+1}, Acc: {acc:.4f}, Loss: {avg_loss:.4f}")
                break
...
if __name__ == "__main__":   
    ...
    CONFIG['type_fusion'] = args.type_fusion
    CONFIG['mode_fusion'] = args.mode_fusion
    ...
    print(f"🎯 FUSION: {CONFIG['type_fusion']}")
    print(f"🎯 MODE FUSION: {CONFIG['mode_fusion']}")
    ...
    MODEL_SAVE_DIR = os.path.join(BASE_PATH, f'results_{safe_name}', CONFIG['type_fusion'] + "_" + CONFIG['mode_fusion'], CONFIG['pooling_type'])
    if not os.path.exists(MODEL_SAVE_DIR): 
        os.makedirs(MODEL_SAVE_DIR)

    # --- [PASSO 1] CAMINHO DO LOG E FUNÇÃO ---
    LOG_TXT_PATH = os.path.join(MODEL_SAVE_DIR, "execution_log.txt")

    def log_this(msg):
        """Imprime na tela (compatível com tqdm) e salva no txt"""
        # 1. Salva no arquivo
        with open(LOG_TXT_PATH, "a", encoding="utf-8") as f:
            f.write(msg + "\n")
        
        # 2. Imprime na tela (tqdm.write não quebra a barra de progresso)
        tqdm.write(msg)
    
    # Cabeçalho do arquivo
    log_this(f"{'='*60}")
    log_this(f"INÍCIO DO TREINO: {CONFIG['model_name']} | {CONFIG['pooling_type']}")
    log_this(f"{'='*60}\n")
    ...
    for task in task_bar:
        ...
        if len(task_weights_runs) > 0:
            ...
            # [PASSO 3] Substitua o print antigo por este bloco:
            mensagem_final = f"✅ {task} Final: {mean_acc:.4f} ± {std_acc:.4f} (Seeds: {len(task_acc_runs)})"
            log_this(mensagem_final)  # <--- AQUI A MÁGICA ACONTECE
